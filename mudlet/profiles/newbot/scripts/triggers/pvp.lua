...
function pvpPolice(line)
	local killerScore = 0
	local victimScore = 0
	local victimID, arenaID, score, eventID, debug, loc
	local killerID, killerSteamOwner, killerUserID, killerPlatform
	...
	if string.find(line, " killed by") then
		...
		killerID, killerSteamOwner, killerUserID, killerPlatform = LookupPlayer(killerName, "all")
	end
	...
	if (killerName == victimName) then
		...
		if (igplayers[killerID].currentLocationPVP or igplayers[victimID].currentLocationPVP) then
			...
			if players[killerID].newPlayer and tonumber(players[killerID].hackerScore) > 50 then
				...
				banPlayer(killerPlatform, killerUserID, killerID, "1 week", "Auto-banned for suspected hacking. Admins have been alerted.", "")
				if botman.dbConnected then conn:execute("INSERT INTO events (x, y, z, serverTime, type, event, steam) VALUES (" .. igplayers[killerID].xPos .. "," .. igplayers[killerID].yPos .. "," .. igplayers[killerID].zPos .. ",'" .. botman.serverTime .. "','ban','Player " .. escape(killerName) .. " temp banned for pvp with a hackerScore > 50','" .. killerID .. "')") end
				return
			end
			...
			if tonumber(players[killerID].playerKills) == 0 then
				...
				if server.allowBank and tonumber(players[victimID].pvpBounty) > 0 then
					message("pm " .. igplayers[killerID].userID .. " [" .. server.chatColour .. "]You won the bounty on " .. victimName .. "![-]")
				end
			end
	...
	if (debug) then dbug("debug pvp line " .. debugger.getinfo(1).currentline) end
			...
			if server.allowBank then
				if tonumber(players[victimID].pvpBounty) > 0 then
					...
					message("pm " .. igplayers[killerID].userID .. " [" .. server.chatColour .. "]You got the bounty on " .. victimName .. "![-]")
				else
	...
	if (debug) then dbug("debug pvp line " .. debugger.getinfo(1).currentline) end
		...
		if isAdminHidden(killerID, killerUserID) and botman.ignoreAdmins then
	...
	if (debug) then dbug("debug pvp line " .. debugger.getinfo(1).currentline) end
			...
			message("pm " .. igplayers[killerID].userID .. " [" .. server.chatColour .. "]You can not return until released from prison.[-]")
			...
			if tonumber(server.maxPrisonTime) > 0 then
				message("pm " .. igplayers[killerID].userID .. " [" .. server.chatColour .. "]You will be automatically released after approximately " .. server.maxPrisonTime .. " minutes.[-]")
			end

			randomTP(killerID, killerUserID, "prison", true)
			...
			if not isAdminHidden(killerID, killerUserID) then
				players[killerID].silentBob = true
			end

			message("pm " .. igplayers[victimID].userID  .. " [" .. server.chatColour .. "]You may release " .. killerName .. ". Do so at your own risk by typing[-]")
			message("pm " .. igplayers[victimID].userID  .. " " .. server.commandPrefix .. "release " .. players[killerID].id .. " or " .. server.commandPrefix .. "release " .. killerName .. "[-]")
			...
			fixMissingPlayer(players[killerID].platform, killerID, killerSteamOwner, killerUserID)
			updatePlayer(killerID)
			saveSQLitePlayer(killerID)
		else
	if (debug) then dbug("debug pvp line " .. debugger.getinfo(1).currentline) end
			if server.gameType == "pve" then
				...
				banPlayer(killerPlatform, killerUserID, killerID, server.pvpTempBanCooldown .. " minutes", "PVP", "")
