...
function playerConnected(line)
	...
	if isReservedName(tmp.player, tmp.steam) then
		...
		irc_chat(server.ircAlerts, "!!  Player joined with admin's name but a different steam key !! " .. tmp.player .. " steam: " .. tmp.steam.. " owner: " .. tmp.steamOwner .. " id: " .. tmp.entityid)
		return
	end
	...
	if (players[tmp.steam] == nil) then
		...
		logChat(botman.serverTime, "Server", "New player joined " .. tmp.player .. " steam: " .. tmp.steam.. " owner: " .. tmp.steamOwner .. " id: " .. tmp.entityid)
