...
-- ################## base command functions ##################
	...
	local function cmd_SetBaseDeadzone()
		if (chatvars.showHelp and not skipHelp) or botman.registerHelp then
			...
			if (chatvars.words[1] == "help" and (string.find(chatvars.command, "set") or string.find(chatvars.command, "base") or string.find(chatvars.command, "dist"))) or chatvars.words[1] ~= "help" then
				irc_chat(chatvars.ircAlias, help[1])
		...
		if string.find(chatvars.command, "set base deadzone") then
			...
			if chatvars.number then
				...
				if chatvars.number == 0 then
					...
					if (chatvars.playername ~= "Server") then
						message(string.format("pm %s [%s]The default base separation of 2x base size will be used.", chatvars.playerid, server.chatColour))
					else
					...
					if (chatvars.playername ~= "Server") then
						message(string.format("pm %s [%s]Players can {#}setbase no closer than " .. server.baseDeadzone .. " metres from non-friended player bases.", chatvars.playerid, server.chatColour))
					else
				...
				if (chatvars.playername ~= "Server") then
					message(string.format("pm %s [%s]A number is required.", chatvars.playerid, server.chatColour))
				else
